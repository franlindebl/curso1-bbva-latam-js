...
class Cliente extends Persona {
	...
	_pedirOrden(carta) {
		let comida = carta._comidas[getRandomInteger(0, carta._comidas.length)];
		let bebida = carta._bebidas[getRandomInteger(0, carta._bebidas.length)];
...
class Restaurante {
	...
	_recibirGrupoClientes() {
		...
		if (copyMesas.length > 0) {
			...
			if (idMesa == null) {
				alert('El grupo se a retirado por que no tenemos mesa disponible.');
			} else {
				let continuar = true;
			...
			alert('No tenemos mesas disponibles.');
		}
	}
