...
class Pokemon {
    constructor(nombre, urlDetalle, id, url_adelante, url_atras) {
        this._nombre = nombre;
        this._urlDetalle = urlDetalle;
        this._id = id;
        this._url_adelante = url_adelante;
        this._url_atras = url_atras;
        //this._imagen = imagen;
        //this._altura = altura;
        //this._peso = peso;
    }
...
///// HEADER
    pintarPaginaPokemones() {

        let main = "";
        main += '<div class="fila-superior" id="divBoton">';
        main += '<h1>La Pokedex Xanxa!</h1>';
        main += '<p id="pagina"></p>';
        main += '<button class="button-main" onclick="" id="boton1">Pagina Actual</button>';
        main += '<button class="button-main" onclick="" id="boton2">Pagina Siguiente</button>';
        main += '</div>';
        main += '<div class="columna-izq">';
        main += this.getEstructuraTablaPersonaje();
        main += '</div>';
        main += '<div class="columna-der">';
        main += this.getDetallePersonaje();
        main += '</div>';
        document.getElementById("pokemon").innerHTML = main;
...
/////////
 ...
 this._pokemones = arrayPokemones;
                ...
                next.addEventListener("click", this.pintarPokemones.bind(this, y));

                console.log("entro aqui", this._pokemones[0]._url_atras);
                var d = this._pokemones[0]._url_atras;
                let back = "";
                back = document.getElementById("boton1");
                back.addEventListener("click", this.pintarPokemones.bind(this, d));
            ...
            tbodypokemones.innerHTML = "";
             this._pokemones.forEach((pokemon) => {
                ...
                //document.querySelector("#btDetalle" + pokemon._urlDetalle).addEventListener("click", this.detallePokemon.bind(this, pokemon._urlDetalle));

                let ide = "";
                ide = document.getElementById(pokemon._id);
                ide.addEventListener("click", this.mostrarDetalle.bind(this, pokemon._urlDetalle));
    ...
    pintarPokemones(url) {
        ...
        let promise = this._pokemonApi.getPokemonsAtPage(url).then(
            (arrayPokemones) => {
                console.log("hola", arrayPokemones);
    ...
    pintar(url) {
        ...
        this.pintarPaginaPokemones();
        this.pintarPokemones(url);
        ...
        var promise = this._apiClient.get(urlCompleta).then(
            (data) => {
                ...
                for (let i = 0; i < data.results.length; i++) {
                    ...
                    let id = i+1;
                    let pokemon = new Pokemon(elem.name, elem.url, id, url_adelante, url_atras);
